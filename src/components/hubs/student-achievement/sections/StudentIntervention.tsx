...
function ProductTile({ item, index }: { item: ProductCard; index: number }) {
  ...
  const isActive = item.id === "student-intervention";
  ...
  return (
    ...
    >
      ...
      {item.subtitle ? (
        <p className="relative z-10 mt-[4px] max-w-[82px] truncate whitespace-nowrap text-[8px] font-normal leading-none text-[color-mix(in_srgb,var(--color-primary)_90%,transparent)]">
          {item.subtitle}
        </p>
      ) : null}
    </motion.button>
