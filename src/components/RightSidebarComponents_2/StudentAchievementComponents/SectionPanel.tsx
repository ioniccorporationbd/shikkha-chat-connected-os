...
export default function SectionPanel({
  ...
  return (
    ...
    >
      <div className="sticky top-4 z-30 bg-[color-mix(in_srgb,var(--color-white)_92%,transparent)] pb-4 pt-1 backdrop-blur-xl supports-[backdrop-filter]:bg-[color-mix(in_srgb,var(--color-white)_78%,transparent)] sm:top-5">
        <div
          className={[
            ...
            "hover:-translate-y-0.5 hover:shadow-[0_24px_60px_color-mix(in_srgb,var(--color-primary)_22%,transparent)] sm:px-5",
            isSolid
        ...
        >
          ...
          <span className="relative z-10 min-w-0">
            <span className="block text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--color-primary)] opacity-70">
              {text.activeProduct}
            </span>
      ...
      <div className="flex min-h-[calc(100vh-112px)] flex-col justify-center pb-10 pt-8 sm:pt-9">
        ...
        <p className="mt-4 max-w-[455px] text-[15px] font-medium leading-[1.65] text-[var(--color-black)] md:text-[16px]">
          {description}
        </p>
        ...
        {stats.length ? (
          <div className="mt-9 space-y-7">
            {stats.map((stat) => (
              <div key={stat.value}>
                <h3 className="text-[44px] font-black leading-none tracking-[-0.06em] text-[var(--color-primary)] sm:text-[46px] md:text-[54px]">
                  {stat.value}
                </h3>
          ...
          <div className="mt-8 overflow-hidden rounded-[18px] bg-[var(--color-secondary)] p-5 shadow-[0_18px_45px_color-mix(in_srgb,var(--color-primary)_14%,transparent)] sm:p-6">
            ...
            <div className="relative mt-6 flex min-h-[130px] items-center justify-center overflow-hidden rounded-2xl">
              ...
              <div className="relative z-10 flex flex-col items-center gap-5 sm:flex-row sm:gap-8">
                ...
                <div className="flex h-[84px] w-[140px] items-center justify-center rounded-xl bg-[color-mix(in_srgb,var(--color-white)_42%,transparent)] px-4 text-center text-[13px] font-black uppercase leading-tight text-[var(--color-black)]">
            ...
            {author ? (
              <div className="mt-6">
                ...
                {role ? (
                  <p className="mt-1 text-[14px] font-medium leading-6 text-[var(--color-black)] opacity-75">
                    {role}
                  </p>
                ) : null}
              </div>
